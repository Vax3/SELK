...
export function prettyInterval(isPaused, intervalInMs) {
  if (isPaused || intervalInMs === 0) {
    ...
    var _units = intervalInSeconds > 1 ? 'seconds' : 'second';

    return "".concat(intervalInSeconds, " ").concat(_units);
  } else if (intervalInMs < MS_IN_HOUR) {
    var intervalInMinutes = Math.round(intervalInMs / MS_IN_MINUTE);

    var _units2 = intervalInMinutes > 1 ? 'minutes' : 'minute';

    return "".concat(intervalInMinutes, " ").concat(_units2);
  } else if (intervalInMs < MS_IN_DAY) {
    var intervalInHours = Math.round(intervalInMs / MS_IN_HOUR);

    var _units3 = intervalInHours > 1 ? 'hours' : 'hour';

    return "".concat(intervalInHours, " ").concat(_units3);
  }

  var intervalInDays = Math.round(intervalInMs / MS_IN_DAY);
  var units = intervalInDays > 1 ? 'days' : 'day';
