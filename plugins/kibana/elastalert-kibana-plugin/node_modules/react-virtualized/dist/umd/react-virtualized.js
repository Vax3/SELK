 ...
 // eslint-disable-line no-new-wrappers
   ...
   */
    function LinearLayoutVector(block_size) {
        ...
        this._defaultSize = 0, this._axisOffset = 0, this._gap = 0, 
        ...
        this.inserts = null, this._length = 0, 
   ...
   */    function inBlockCopy(dstBlock, dstIndexStart, srcBlock, srcIndexStart, count) {
        ...
        srcValue !== (dstValue = dstSizes[dstIndex]) && (// Triple '=' to handle NaN comparison
        // Are we removing a default size or a chached size?
        dstValue != dstValue ? defaultCountDelta-- : sizesSumDelta -= dstValue, 
   ...
   */    function copyInterval(dstIndex, srcIndex, count) {
        ...
        for (ascending || (dstIndex += count - 1, srcIndex += count - 1); count > 0; ) {
            ...
            this.block_table[dstBlockIndex] = dstBlock), dstBlock) 
            // If this is the last time we're visiting this block, and it contains
            // only NaNs, then remove it
            if (inBlockCopy(dstBlock, dstStartIndex, srcBlock, srcStartIndex, copyCount), dstBlock.defaultCount === this.block_size) ((ascending ? dstStartIndex + copyCount === this.block_size : 0 === dstStartIndex) || count === copyCount) && (this.block_table[dstBlockIndex] = null);
   ...
   */    function clearInterval(start, end) {
        for (;start <= end; ) {
            ...
            clearCount = Math.min(clearCount, end - start + 1), block && (clearCount === this.block_size ? this.block_table[blockIndex] = null : (
        ...
        function(intervals) {
            ...
            if (0 !== intervalsCount) {
                ...
                do {
                    ...
                    count = nextIntervalStart - (
                    // Start copy from after the end of current interval
                    srcStart = intervalEnd + 1), copyInterval.call(this, dstStart, srcStart, count), 
                    dstStart += count;
   ...
   */ .call(this, intervals); else if (this.inserts) {
            intervals = this.inserts;
            var newLength = this._pendingLength;
            this.inserts = null, this._pendingLength = -1, function(intervals, newLength) {
                var intervalsCount = intervals.length;
                if (0 !== intervalsCount) {
   ...
   */
    ...
    LinearLayoutVector.prototype.getLength = function() {
        return -1 === this._pendingLength ? this._length : this._pendingLength;
    }
    /**
   * @private
   ...
   */ , LinearLayoutVector.prototype.setLength = function(newLength) {
   ...
   *
   * @default 0
   */ , LinearLayoutVector.prototype.getDefaultSize = function() {
        return this._defaultSize;
    }
    /**
   * @private
   */ , LinearLayoutVector.prototype.setDefaultSize = function(value) {
   ...
   *
   * @see #gap
   */ , LinearLayoutVector.prototype.getAxisOffset = function() {
        return this._axisOffset;
    }
    /**
   * @private
   */ , LinearLayoutVector.prototype.setAxisOffset = function(value) {
   ...
   *
   * @default 0
   */ , LinearLayoutVector.prototype.getGap = function() {
        return this._gap;
    }
    /**
   * @private
   */ , LinearLayoutVector.prototype.setGap = function(value) {
        this._gap = value;
    }
    //--------------------------------------------------------------------------
    //
   ...
   * @param index The item's index.
   * @see defaultSize
   */ , LinearLayoutVector.prototype.getItemSize = function(index) {
   ...
   * @see defaultSize
   */ , LinearLayoutVector.prototype.setItemSize = function(index, value) {
        ...
        oldValue != oldValue ? (block.defaultCount -= 1, block.sizesSum += value) : value != value ? (block.defaultCount += 1, 
        block.sizesSum -= oldValue) : block.sizesSum += value - oldValue;
   ...
   */ , LinearLayoutVector.prototype.insert = function(index) {
        // We don't support interleaved pending inserts and removes
        this.removes && flushPendingChanges.call(this);
        if (this.inserts) {
            ...
            this.inserts[lastIndex] = index : index > intervalEnd ? (
            // New interval
   ...
   */ , LinearLayoutVector.prototype.remove = function(index) {
        // We don't support interleaved pending inserts and removes
        this.inserts && flushPendingChanges.call(this);
        // length getter takes into account pending inserts/removes but doesn't flush
                if (index >= this.getLength()) throw new Error("Invalid index and all that.");
        if (this.removes) {
            // Update the last interval or add a new one?
            var lastIndex = this.removes.length - 1, intervalStart = this.removes[lastIndex];
            index === intervalStart - 1 ? 
            // Extend the start of the interval
            this.removes[lastIndex] = index : index < intervalStart ? (
            // New interval
            this.removes.push(index), this.removes.push(index)) : 
            // We can't support pending removes that are not descending
            flushPendingChanges.call(this);
        }
        this._pendingLength = -1 === this._pendingLength ? length - 1 : this._pendingLength - 1, 
        this.removes || (this.removes = [], this.removes.push(index), this.removes.push(index));
    }, LinearLayoutVector.prototype.start = function(index) {
   ...
   * @param index The item's index.
   * @see #start
   */ , LinearLayoutVector.prototype.end = function(index) {
   ...
   */ , LinearLayoutVector.prototype.indexOf = function(distance) {
        flushPendingChanges.call(this);
        var index = function(distance) {
            ...
            for (var index = -1, block = null, blockSize = this.block_size, defaultSize = this._defaultSize, gap = this.getGap(), blockGap = gap * blockSize, blockIndex = -1, blockTable = this.block_table, blockTableLength = blockTable.length
            // Find the block that contains distance and the index of its
            // first element
            ; ++blockIndex < blockTableLength; ) {
            ...
            if (-1 === index || distance === curDistance) return index;
            // At this point index corresponds to the first item in this block
                        if (block) {
                for (
                // Find the item that contains distance and return its index
                var sizes = block.sizes, n = this.block_size - 1, i = 0; i < n; i++) {
                    var size = sizes[i];
                    if ((curDistance += gap + (size != size ? this._defaultSize : size)) > distance) return index + i;
                }
   ...
   */ .call(this, distance);
    ...
    }, LinearLayoutVector.prototype.clear = function() {
    ...
    }, LinearLayoutVector.prototype.toString = function() {
    ...
    var CellSizeAndPositionManager = function() {
        function CellSizeAndPositionManager(_ref) {
            ...
            classCallCheck(this, CellSizeAndPositionManager), this._lastMeasuredIndex = -1, 
            this._cellSizeGetter = cellSizeGetter, this._cellCount = cellCount, this._estimatedCellSize = estimatedCellSize, 
            this._layoutVector = new linearLayoutVector(), this._layoutVector.setLength(cellCount), 
        ...
        return createClass(CellSizeAndPositionManager, [ {
            ...
            value: function(offset) {
                ...
                for (var vector = this._layoutVector, lastIndex = this._cellCount - 1, targetOffset = Math.max(0, Math.min(offset, vector.start(lastIndex))), nearestCellIndex = vector.indexOf(targetOffset); nearestCellIndex > this._lastMeasuredIndex; ) {
                    if (this.getSizeAndPositionOfCell(nearestCellIndex), nearestCellIndex === lastIndex) return nearestCellIndex;
                    -1 === (nearestCellIndex = vector.indexOf(targetOffset)) && (nearestCellIndex = this._lastMeasuredIndex, 
                    this._lastMeasuredIndex = nearestCellIndex - 1, targetOffset = Math.max(0, Math.min(offset, vector.start(lastIndex))));
                }
